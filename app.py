...
def load_models():
    ...
    with _model_lock:
        ...
        _models_loaded = True
        logger.info(f"Pipeline ready — {len(KNOWLEDGE_BASE)} concepts loaded.")
...
def evaluate_understanding(user_understanding, concept):
    ...
    return {"label": "CORRECT" if similarity >= CORRECT_THRESHOLD else "INCORRECT", "similarity": round(similarity, 4)}
...
def append_to_session(session_id, messages):
    with _session_lock:
        if session_id not in sessions:
            sessions[session_id] = []
        sessions[session_id].extend(messages)
        cap = MAX_SESSION_TURNS * 2
        if len(sessions[session_id]) > cap:
            sessions[session_id] = sessions[session_id][-cap:]
...
def chat():
    ...
    if not question or not understanding:
        return jsonify({"error": "Both 'question' and 'understanding' fields are required."}), 400
    if len(question) > MAX_INPUT_LENGTH or len(understanding) > MAX_INPUT_LENGTH:
        return jsonify({"error": f"Input too long. Maximum {MAX_INPUT_LENGTH} characters per field."}), 400

    result = run_pipeline(question, understanding)

    if result["label"] == "OUT_OF_SCOPE":
        return jsonify({"response": "I only engage with RNN-related academic concepts. Ask me something worth disagreeing about.", "label": "OUT_OF_SCOPE", "similarity": None, "tone": None, "concept": None})
    ...
    try:
        ...
        return jsonify({"error": "LLM service unavailable. Please try again."}), 503
    ...
    return jsonify({"response": reply, "label": result["label"], "similarity": result["similarity"], "tone": result["tone"], "concept": result["concept"]})
